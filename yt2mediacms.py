...
def update_mediacms_channel(mediacms_url, token, channel_info, mediacms_username):
    ...
    headers = {
        ...
        # Optionally include X-CSRFTOKEN header if needed.
    }
    
    description_value = (
        f"{channel_info['channel_description']}"
    )
    
    # Prepare multipart fields. For text fields, using (None, value) sends them as plain text.
    files = {
    ...
    try:
        ...
        # Close logo file if it's a file-like object.
...
def main():
    ...
    parser.add_argument('--yt-api-key', required=True, help='YouTube API key')
